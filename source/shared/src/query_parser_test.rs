...
fn test_default_albums() {
    ...
    compile_query(&read_to_string(query_dir.join("query_audio_albums.txt")).unwrap()).unwrap();
...
fn test_default_albums_tracks() {
    let query_dir = src_query_dir();
    compile_query(&read_to_string(query_dir.join("query_audio_tracks_search_name.txt")).unwrap()).unwrap();
    compile_query(&read_to_string(query_dir.join("query_audio_tracks_suffix.txt")).unwrap()).unwrap();
}

#[test]
fn test_default_notes() {
    let query_dir = src_query_dir();
    compile_query(&read_to_string(query_dir.join("query_notes_by_add_date.txt")).unwrap()).unwrap();
    compile_query(&read_to_string(query_dir.join("query_notes_suffix.txt")).unwrap()).unwrap();
}
