...
in
{
  extensionUnpacked = pkgs.runCommand "sunwet-browser-unpacked" { } ''
    ...
    ${shared.nativeBindgen}/bin/bind_wasm --in-wasm ${wasmUnbound}/bin/browser-background.wasm --out-name background2 --out-dir browser_wasm
    ...
    hoj_cp browser_wasm/background2.d.ts browser_src/ext_static/background2.d.ts
