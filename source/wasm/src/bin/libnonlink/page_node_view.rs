...
fn build_node_rel_buttons(eg: &EventGraph, node: &Node, download: Option<&FileHash>, history: String) -> El {
    let mut right = vec![];
    ...
    setup_node_button(eg, &node_button, node_to_text(&node), node.clone());
...
pub fn build_page_node_view(pc: &mut ProcessingContext, title: &str, node: &Node) {
    set_page(pc, title, el_async_(true, {
        ...
        async move {
            ...
            {
                let node_button = style_export::leaf_node_view_toolbar_node_button().root;
                setup_node_button(&eg, &node_button, title.clone(), node.clone());
