...
pub fn build(input: BuildDbInput) -> (Version, Vec<Query>) {
    ...
    // Triple (old table, kept for migration data source, no queries)
    {
        let t = version.table("zQLEK3CT0", "triple");
        let subject = t.field(&mut version, "zLQI9HQUQ", "subject", FieldType::with(&input.node_type));
        let predicate = t.field(&mut version, "zSZVNBP0E", "predicate", field_str().build());
        let object = t.field(&mut version, "zII52SWQB", "object", FieldType::with(&input.node_type));
        let commit = t.field(&mut version, "zK21ECBE5", "commit_", field_utctime_ms_chrono().build());
        let exist = t.field(&mut version, "z0ZOJM2UT", "exists", field_bool().build());
        t.constraint(
            &mut version,
            "z1T10QI43",
            "triple_pk",
            PrimaryKey(PrimaryKeyDef { fields: vec![subject.clone(), predicate.clone(), object.clone(), commit.clone()] }),
        );
        t
            .index("zXIMPRLIR", "triple_index_obj_pred_subj", &[&object, &predicate, &subject, &commit])
            .unique()
            .build(&mut version);
        t.index("zBZVX51AR", "triple_index_pred_subj", &[&predicate, &subject, &commit]).build(&mut version);
        t.index("zTVLKA6GQ", "triple_index_pred_obj", &[&predicate, &object, &commit]).build(&mut version);
        t.index("woeehiw2a9lszj", "triple_commit_exists", &[&commit, &exist]).build(&mut version);
    }
    ...
    // Triple2 (normalized history table, replaces triple)
    let triple2_table;
    let triple2_commit;
    let triple2_subject;
